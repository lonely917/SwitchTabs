...
public class AsynImageLoader {
	...
	private Bitmap defaultBitmap, errorBitmap; // 默认图片和错误图片

	public AsynImageLoader(Handler handler, Bitmap defaultBitmap, Bitmap errorBitmap) {
	...
	public void loadBitmap(ImageView imageView) {
		...
		if (imageCache.containsKey(url)) {// 判断缓存中是否有
			...
			if (bitmap != null) {// 如果图片对象不为空，则可直接更新视图，并返回
		...
		if (thread == null && url != null) {// 加载线程不存在，线程还未启动，需要新建线程并启动
			thread = new LoaderThread(imageView, url);
			thread.start();
		} else if (url != null) {// 如果存在，就调用线程对象去加载
			thread.load(imageView, url);
		} else {// 这种情况就是没有url地址，直接显示错误
	...
	private class LoaderThread extends Thread {
		...
		public void run() {
			while (mTaskMap.size() > 0) {// 当队列中有数据时线程就要一直运行,一旦进入就要保证其不会跳出循环
				...
				if (url != null && imageView.getTag() == url) {// 判断视图有没有复用（一旦ImageView被复用，其tag值就会修改变）
					final Bitmap bitmap = Utils.getBitmapByUrl(url);// 此方法应该是从网络或sd卡中加载
					// 将加载的图片放入缓存map中
					imageCache.put(url, new SoftReference<Bitmap>(bitmap));
					if (url.equals(imageView.getTag())) {// 再次判断视图有没有复用
